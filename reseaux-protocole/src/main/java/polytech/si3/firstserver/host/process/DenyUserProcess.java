 ...
 */
public class DenyUserProcess extends IdeaProcess {

    private UserData userInterested;
    private int ideaId;
    private String userId;

    public DenyUserProcess(Users users, String adress, Ideas ideas, String userId, int ideaId) {
...
//        System.out.println(idea.authorId() + " : " + idea.managerId());
        ...
        if(idea.getParticipant(userId,users) != null){
            ...
            if(idea.getParticipant(userId,users).getStatus() == 1){
                idea.getParticipant(userId,users).setStatus(0);
                idea.removeParticipant(userId);
                return new Success();
            }
